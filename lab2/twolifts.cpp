...
This Program helps you to simulate multiple Elevators
The Problem Statement needed it for only 2, but in here
you could do so for many

Also You could do so for as many Floors as you want!
*/
...
int ElevatorCount;                //Number of Lifts
...
float MaxElevatorCapacity = -5;   //To keep track of Max-capacity across all Elevators
...
public:
  ...
  //To Get the Total Number of Passengers presently in the lift
  ...
  //Getting A passenger who wants to Go UP
  void GetPassengerUp(){
  ...
  //Getting a Passenger who wants to go down
...
//Class for Controlling all aspects of the Simulation
class Controller{
public:
  ...
  void PrintElevatorStatus() {
    ...
    for(int i = 0;i<ElevatorCount;i++){
      cout<<i<<": Current Weight is "<<Elevators[i].getWeight()<<
      ", Total Passenger Count is "<<Elevators[i].getTotalPassengerCount()<<
      ", The Lift is currently on Floor "<<Elevators[i].getCurrentFloor()<<
      ", And the Lift is moving ";
      if(Elevators[i].getDirection())cout<<"down\n";
      else cout<<"up \n";
    }
  ...
  void Initialize() {
    ...
    cout<<"Kindly enter the Number of Elevators you'd like to have:"<<endl;
    cin>>ElevatorCount;                      //Number of Elevators

    Elevators = new Lift[ElevatorCount];
    ...
    for (int i = 0; i<ElevatorCount; i++) {
      ...
      cout<<"Kindly Enter the capacity of the "<<i+1<<"th Elevator:"<<endl;
      cin>>c;
      ...
      if (c > MaxElevatorCapacity) {
        MaxElevatorCapacity = c;
      }
      cout<<"Kindly Enter the initial floor of the "<<i+1<<"th Elevator"<<endl;
      cin>>curfloor;
      ...
      Elevators[i] = Lift(curfloor,c);      //Constructing the lifts
    }
  ...
  void Play(){
    ...
    bool play = true;           //To Decide When to end the Simulation

    while ( play ) {

      PrintInstructions();      //Print Guidelines
      int what;                 //Variable to Decide, whatever the user wants to do
      cin>>what;
      switch (what) {
        ...
        case 3:
          ...
          //Moving the Elevators for a certain number of steps!
          cout<<"For How many steps would you want to move: "<<endl;
          cin>>Steps;

          for ( int g = 0; g<Steps; g++ ) {
            for ( int f = 0; f<ElevatorCount; f++) {
              Elevators[f].NextMove(1);
            }
          }

          //Printing Status after that
